package us.forestbukkit.kitpvp.controller;
...
public class ClassRegistrationController {
	...
	public void loadListeners(String packageName) {
		for (Class<?> clazz : getClassesInPackage(packageName)) {
			if (isListener(clazz)) {
				try {
					KitPvP.getInstance().getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), KitPvP.getInstance());
	...
	public Collection<Class<?>> getClassesInPackage(String packageName) {
		...
		CodeSource codeSource = KitPvP.getInstance().getClass().getProtectionDomain().getCodeSource();
